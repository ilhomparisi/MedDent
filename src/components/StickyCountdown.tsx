  ...
  const fetchExpirySettings = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('key, value')
        .in('key', [
          'countdown_expiry_text', 'countdown_expiry_text_uz', 'countdown_expiry_text_ru',
          'countdown_expiry_text_size', 'countdown_expiry_text_weight', 'countdown_expiry_text_align'
        ]);
      ...
      data?.forEach((setting) => {
        if (setting.key === 'countdown_expiry_text') {
          textDefault = setting.value;
        } else if (setting.key === 'countdown_expiry_text_uz') {
          textUz = setting.value;
        } else if (setting.key === 'countdown_expiry_text_ru') {
          textRu = setting.value;
        } else if (setting.key === 'countdown_expiry_text_size') {
          setExpiryTextSize(setting.value);
        } else if (setting.key === 'countdown_expiry_text_weight') {
          setExpiryTextWeight(setting.value);
        } else if (setting.key === 'countdown_expiry_text_align') {
          setExpiryTextAlign(setting.value);
        }
      });

      const selectedText = language === 'ru' && textRu ? textRu :
                          language === 'uz' && textUz ? textUz :
                          textDefault;
      setExpiryText(selectedText);
    } catch (error) {
      console.error('Error fetching expiry settings:', error);
    }
  };

  const fetchCountdownGlowSettings = async () => {
    try {
      const { data, error } = await supabase
        .from('site_settings')
        .select('key, value')
        .in('key', ['countdown_glow_text', 'countdown_glow_text_uz', 'countdown_glow_text_ru', 'countdown_glow_color', 'countdown_glow_intensity']);
      ...
      data?.forEach((setting) => {
        if (setting.key === 'countdown_glow_text') {
          glowTextDefault = setting.value;
        } else if (setting.key === 'countdown_glow_text_uz') {
          glowTextUz = setting.value;
        } else if (setting.key === 'countdown_glow_text_ru') {
          glowTextRu = setting.value;
        } else if (setting.key === 'countdown_glow_color') {
          setCountdownGlowColor(setting.value);
        } else if (setting.key === 'countdown_glow_intensity') {
          setCountdownGlowIntensity(setting.value);
        }
      });

      const selectedGlowText = language === 'ru' && glowTextRu ? glowTextRu :
                               language === 'uz' && glowTextUz ? glowTextUz :
                               glowTextDefault;
      setCountdownGlowText(selectedGlowText);
    } catch (error) {
      console.error('Error fetching countdown glow settings:', error);
    }
  };
  ...
  const fetchSettings = async () => {
    try {
      const { data, error } = await supabase
        .from('site_settings')
        .select('key, value')
        .in('key', ['offer_enabled', 'offer_hours']);
      ...
      if (settingsObj.offer_enabled !== undefined) {
        setOfferEnabled(settingsObj.offer_enabled);
      }
      if (settingsObj.offer_hours !== undefined) {
        setOfferHours(settingsObj.offer_hours);
      }
    } catch (error) {
  ...
  const fetchUITexts = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('key, value')
        .eq('section', 'hero')
        .in('key', ['countdown_label']);

      if (error) throw error;

      data?.forEach((item) => {
        if (item.key === 'countdown_label') {
          setCountdownLabel(item.value);
        }
      });
    } catch (error) {
